...
仅迁移模板自身的视图、脚本、样式、字体、图片和视频。文章、分类、账号、线索、表单定义、站点配置和首页模块配置保留在各实例。客户私有备注属于交付文件的一部分；交付者继续负责客户素材的使用和发布范围。
...
ZIP 默认上限 32 MiB、500 个文件、视频单文件 25 MiB、其他单文件 5 MiB、解包总量 40 MiB，可在 `config/geoflow.php` 的 `theme_packages` 中调整。上传页会显示 PHP 与应用共同允许的有效 ZIP 上限。`package.json` 另限 1 MiB。视频仅允许 MP4（H.264）、WebM（VP8、VP9、AV1）和 Ogg（Theora），导入与导出均使用 ffprobe 检查容器、视频轨、时长（不超过 120 秒）、帧率（不超过 60 fps）和分辨率（不超过 1920×1080）。模板中可使用 `<x-site.video :src="..." :poster="..." />`，组件在点击前不会请求视频文件。
...
视频访问使用 5 分钟公共缓存，并保留 HTTP Range 分段请求与跳播。Nginx 对视频路径按客户端 IP 限制为平均每秒 2 次、突发 10 次和同时 4 个请求；超过限制返回 429。此限制需要在实际部署中观察日志与共享出口 IP 的使用情况。若有 CDN 或反向代理，须先配置可信代理地址，确保 Nginx 获取真实客户端 IP；不可直接信任任意客户端提供的转发头。模板视频播放组件初始只呈现封面，点击后才为 video 元素设置资源地址。

上传 32 MiB ZIP 时，PHP `upload_max_filesize` 至少需要 32 MiB，`post_max_size` 与 Nginx `client_max_body_size` 应更高。官方容器已设置 PHP 40 MiB 和 Nginx 64 MiB。视频校验要求服务器安装 ffprobe；官方 PHP 容器会安装 ffmpeg 包。导入、解包、安装及导出时会保留多份临时副本，部署时应预留相应磁盘空间。
